  ...
  <?php
    if (isset($_POST['tambahdata']))
    {
      ...
      $querycek   =  mysqli_query($con, "SELECT * FROM tbl_mahasiswa WHERE nim ='$nim'") or die (mysqli_error($con));
       ...
       {
           echo "<script>alert('Maaf, Transaksi Gagal! Data Mahasiswa yang diinput sudah ada..');</script>";
           echo "<script>window.location='../admin_backend_mahasiswa';</script>";

       }
      else
       {
           mysqli_query($con, "INSERT INTO tbl_mahasiswa VALUES ('$nim','$nama','$kelamin','$kontak','$status','$foto')") or die (mysqli_error($con));
